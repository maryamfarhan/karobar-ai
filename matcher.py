...
class ProviderMatcher:
    ...
    def find_providers(self, service, city, area=None, max_price=None, require_available=False):
        ...
        for p in self.providers:
            ...
            # Proximity / Area Match component (15%)
            area_match = False
            if area and p['area'].lower() == area.lower():
                area_match = True
            s_area = 0.15 if area_match else 0.00
            
            # Score Components
            s_rating = (p['rating'] / 5.0) * 0.25
            s_on_time = p['on_time_score'] * 0.20
            s_cancel = (1.0 - p['cancellation_rate']) * 0.15
            ...
            total_score = s_area + s_rating + s_on_time + s_cancel + s_exp + s_price + s_avail
            
            match = p.copy()
            match['match_score'] = round(total_score * 100, 2)
            match['score_breakdown'] = {
                'area_score': round(s_area * 100, 2),
